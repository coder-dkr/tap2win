...
const getStats = async (req, res) => {
  try {
    ...
    const completedAuctionsData = await Auction.findAll({
      ...
      include: [
        { model: Bid, as: "bids", order: [["amount", "DESC"]], limit: 1 },
      ],
    });
    ...
    res
      .status(500)
      .json({ success: false, message: "Failed to get admin statistics" });
  }
};
...
const startAuction = async (req, res) => {
  try {
    ...
    if (auction.status !== "pending") {
      return res
        .status(400)
        .json({ success: false, message: "Auction is not in pending status" });
    }
    ...
    res
      .status(500)
      .json({ success: false, message: "Failed to start auction" });
  }
};
...
const resetAuction = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: "Failed to reset auction" });
  }
};
...
const updateAuction = async (req, res) => {
  try {
    const { id } = req.params;
    const { title, description, startingPrice, bidIncrement, endTime } =
      req.body;
    const auction = await Auction.findByPk(id);
    if (!auction) {
      return res
  ...
  try {
    ...
    if (auction.status === "active" || auction.status === "ended") {
      return res
        ...
        .json({
          success: false,
          message: "Cannot delete active or ended auction",
        });
    }
    await Bid.destroy({ where: { auctionId: id } });
...
const getSystemStatus = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: "Failed to get system status" });
  }
};
